...
class _SearchPageState extends State<SearchPage> {
  ...
  void _showFilterSheet() {
    // temp selection
    final tempKategori = Rx<Map<String, dynamic>?>(_ctrl.selectedKategori.value);
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  if (kat != null) _activeChip(kat['name'], () => _ctrl.applyFilter(genre: gen)),
                  if (kat != null && gen != null) const SizedBox(width: 6),
                  if (gen != null) _activeChip(gen['name'], () => _ctrl.applyFilter(kategori: kat)),
                  const Spacer(),
                  GestureDetector(
...
class _BookResultItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isPopuler = book.totalDipinjam > 0 && book.rating >= 4.0 && book.totalRating >= 3;
    ...
    final catData = categories.firstWhereOrNull((c) => c['name'] == book.kategori);
    final catColor = catData != null ? _parseColor(catData['color'] ?? '#1565C0') : const Color(0xFF1565C0);
    final genreNames = book.genre.split(',').map((g) => g.trim()).where((g) => g.isNotEmpty).toList();
    final genreDataList = genreNames.map((name) =>
        genres.firstWhereOrNull((g) => (g['name'] as String).toLowerCase() == name.toLowerCase())
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: const Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.local_fire_department, color: Colors.white, size: 9),
                            SizedBox(width: 2),
                            Text('Populer',
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(tersedia ? 'Tersedia' : 'Dipinjam',
                          style: TextStyle(
                ...
                Row(
                  children: [
                    ...
                    Text('${book.rating.toStringAsFixed(1)} (${book.totalRating})',
  ...
  Widget _placeholder() => Container(
        ...
        child: const Center(child: Icon(Icons.menu_book, color: Colors.black26, size: 32)),
      );
}
