...
class ProfileService extends GetConnect {
  ...
  Future<Map<String, dynamic>> getBorrowings({String status = 'dipinjam', int limit = 5}) async {
    final res = await get('/api/borrowings/borrowings?status=$status&page=1&limit=$limit');
    if (res.statusCode == 200) return res.body;
  ...
  Future<String> uploadPhoto(List<int> bytes, String filename) async {
    final form = FormData({'photo': MultipartFile(bytes, filename: filename, contentType: 'image/jpeg')});
  ...
  Future<void> requestOtp() async {
    ...
    if (res.statusCode != 200) throw Exception(res.body['message'] ?? 'Gagal kirim OTP');
  }
